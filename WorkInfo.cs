...
namespace DAL
{
    public class WorkInfo
    {
        ...
        public Model.WorkInfoNo GetModel(int id)
        {
            ...
            {
                string sql = "select WorkInfo.*,activity.activityName from workInfo jion activity on workInfo.activityId=activity.activityId where workId=@id";
        ...
        public IEnumerable<Model.WorkInfo> GetRecommend()
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "select * from workinfo where workVerify='审核通过'and recommend='是' order by uploadTime desc limit 6";
                return cn.Query<Model.WorkInfo>(sql);
            }
        }
        public int GetCount()
        ...
        public IEnumerable<Model.WorkInfoNo> GetPage(Model.WorkPage page)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                ...
                return cn.Query<Model.WorkInfoNo>(sql, new { pageIndex = page.pageIndex, pageSize = page.pageSize, activityId = page.activityIds });
            }
        }
        ...
        public int GetMyCount(string userName)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "delete from workinfo where userName=@userName";
        ...
        public int Add(Model.WorkInfo workinfo)
        {
            using (IDbConnection cn = new MySqlConnection(cns))
            {
                string sql = "insert into workinfo(workId,workName,workPicture,uopoadTime,workIntroduction,workVerify,userName,activityId,recommend,recommendTime)"+ "values(@workId,@workName,@workPicture,@uploadTime,@workIntroduction,@workVerify,@userName,@activityId,@recommend,@recommendTime);";
